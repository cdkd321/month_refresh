...
class OrderCommitView(View):
    ...
    def post(self, request):
        ...
        save_trans_id = transaction.savepoint()
                    ...
                    try:
                        ...
                        if ret == 0:
                            ...
                            if i < 2:
                                continue
                        transaction.savepoint_rollback(save_trans_id)
                        return JsonResponse({'res': 8, 'message': '下单失败，库存出现不足'})
        ...
        transaction.savepoint_commit(save_trans_id  )
    ...
    def post(self, request):
        ...
        try:
            ...
            for sku_id in sku_ids:
                ...
                print('addr_id:%s, pay_method:%s, sku_ids:%s' % (addr_id, pay_method, sku_ids))
                print('uid:%s,stock:%s' % (user.id, sku.stock))
